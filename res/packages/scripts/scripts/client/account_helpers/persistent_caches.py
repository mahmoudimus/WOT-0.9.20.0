# 2017.08.29 21:43:18 Støední Evropa (letní èas)
...
class SimpleCache(object):
    ...
    def getFileName(self):
        raise self.__account is not None or AssertionError
    ...
    def save(self, descr, data):
        if not self.__account is not None:
            raise AssertionError
            return self.__account is None and None
        else:
            try:
                fileName = self.getFileName()
                self.__memcache[fileName] = (descr, data)
...
def writeFile(filename, data):
    ...
    return True
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\account_helpers\persistent_caches.pyc 
