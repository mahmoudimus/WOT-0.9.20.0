# 2017.08.29 21:43:28 Støední Evropa (letní èas)
...
class _Observable(object):
    ...
    def subscribe(self, subscriber):
        ...
        """
        raise subscriber not in self.__subscribers or AssertionError
...
class _GlobalDataDescriptor(object):
    ...
    def __init__(self, bindingID, reader = False, writer = False):
        ...
        if not bindingID in BINDING_ID.RANGE:
            raise AssertionError
            self.__bindingID = bindingID
            self.__reader = reader
            self.__writer = writer
            self.__storage[bindingID] = bindingID not in self.__storage and _DEFAULT_VALUES[bindingID]()
    ...
    def subscribe(cls, bindingID, subscriber):
        raise bindingID in cls.__storage or AssertionError('The bindingID {} is not found in the storage'.format(bindingID))
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\AvatarInputHandler\aih_global_binding.pyc 
