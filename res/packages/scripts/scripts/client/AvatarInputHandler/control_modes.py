# 2017.08.29 21:43:29 Støední Evropa (letní èas)
...
class _GunControlMode(IControlMode):
    ...
    def getDesiredShotPoint(self, ignoreAimingMode = False):
        if not self._isEnabled:
            ...
            return (self._aimingMode == 0 and self._cam is not None or ignoreAimingMode) and self._cam.aimingSystem.getDesiredShotPoint()
        else:
            return None
...
class PostMortemControlMode(IControlMode):
    ...
    def __switchToViewpoint(self, toId):
        if self.__postmortemDelay is not None:
            ...
            raise isinstance(toId, int) or AssertionError
            self.__doPreBind()
    ...
    def __switch(self, isNext = True):
        if self.__postmortemDelay is not None:
            ...
            raise isinstance(isNext, bool) or AssertionError
            self.__doPreBind()
...
def _swap(data, index1, index2):
    ...
    data[index2] = tmp
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\AvatarInputHandler\control_modes.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:43:31 Støední Evropa (letní èas)
