# 2017.08.29 21:51:20 Støední Evropa (letní èas)
...
class SubPendingItem(ContactItem):
    ...
    def replace(self, newItem):
        ...
        """
        if newItem and newItem.getItemType() == XMPP_ITEM_TYPE.TMP_BLOCK_ITEM:
            return SubPendingTmpBlockItem(self._jid, SubPendingItem(self._jid, trusted=self.isTrusted(), tags=self.getTags(), receivedAt=self.receivedAt()))
        return super(SubPendingItem, self).replace(newItem)
...
def createItem(databaseID, itemType = XMPP_ITEM_TYPE.EMPTY_ITEM, trusted = True):
    ...
    return item
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\messenger\proto\xmpp\xmpp_items.pyc 
