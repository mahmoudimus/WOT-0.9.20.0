# 2017.08.29 21:43:43 Støední Evropa (letní èas)
...
class Bootcamp(EventSystemEntity):
    ...
    def hookOnEnterWorld(self):
        if self.__currentState is None or self.__currentState.id() != STATE.BATTLE_PREPARING:
            return False
        else:
            return self.__currentState.hookOnEnterWorld()

    def hookVehicleOnEnterWorld(self):
        if self.__currentState is None or self.__currentState.id() != STATE.BATTLE_PREPARING:
            return False
        else:
            return self.__currentState.hookVehicleOnEnterWorld()
    ...
    def getIntroVideoData(self):
        if self.__currentState:
            return self.__currentState.getIntroVideoData()
        return {}
...
g_bootcamp = Bootcamp()
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\bootcamp\Bootcamp.pyc 
