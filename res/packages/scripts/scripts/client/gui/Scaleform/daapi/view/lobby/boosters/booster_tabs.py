# 2017.08.29 21:46:41 Støední Evropa (letní èas)
...
class ShopBoostersTab(BoosterTab):
    ...
    def __getActionVO(booster):
        ...
        if itemPrice.isActionPrice():
            return packActionTooltipData(ACTION_TOOLTIPS_TYPE.BOOSTER, str(booster.boosterID), True, itemPrice.price, itemPrice.defPrice)
        else:
            return None
    ...
    def __getPriceState(isPurchaseEnabled):
        if isPurchaseEnabled:
            return _PRICE_STATE.NORMAL
        return _PRICE_STATE.ERROR
...
def _makeBoosterSlotVO(booster, count):
    return {'boosterId': booster.boosterID,
     ...
     'showLeftTime': False}
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\boosters\booster_tabs.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:46:42 Støední Evropa (letní èas)
