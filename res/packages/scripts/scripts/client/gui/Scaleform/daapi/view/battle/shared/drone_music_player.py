# 2017.08.29 21:46:04 Støední Evropa (letní èas)
...
def _delegate(func):
    ...
    def __wrapper(self, *args, **kwargs):
        raise isinstance(self, DroneMusicPlayer) or AssertionError
...
def _initCondition(func):
    ...
    @wraps(func)
    def __wrapper(self, *args, **kwargs):
        raise isinstance(self, _Condition) or AssertionError
        ...
        for points in self.__pointsToBase.itervalues():
            if self.__stopCapturingCooldown is not None and points:
                if not self.isSatisfied():
                    raise AssertionError('Condition must be in the valid state while cooldown is working')
                    BigWorld.cancelCallback(self.__stopCapturingCooldown)
                    LOG_DEBUG('Base Capturing. Cooldown stopped')
                    self.__stopCapturingCooldown = None
                    return False
                return points >= criticalPointsCount and self._updateValidValue(True)

        if not (self.isSatisfied() and self.__stopCapturingCooldown is None):
            raise AssertionError('Cooldown must be set only once!')
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\battle\shared\drone_music_player.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:46:05 Støední Evropa (letní èas)
