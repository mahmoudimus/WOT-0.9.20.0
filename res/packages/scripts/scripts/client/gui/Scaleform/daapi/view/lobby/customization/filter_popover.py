# 2017.08.29 21:46:50 Støední Evropa (letní èas)
...
class FilterPopover(CustomizationFiltersPopoverMeta):
    ...
    def _populate(self):
        ...
        for cType in CUSTOMIZATION_TYPE.ALL:
            ...
            group.sort(key=lambda (grType, _):             if grType in GROUP_NAMES_FILTER_ORDER:
GROUP_NAMES_FILTER_ORDER_INDICES[grType](OTHER_GROUP_NAMES_FILTER_ORDER if grType == OTHER_GROUP_NAME else UNKNOWN_GROUP_NAMES_FILTER_ORDER))
    ...
    def __switchIGRFilter(self, disableGroupFilter):
        ...
        if self.__filter.isGroupFilterEnabled() == disableGroupFilter:
            ...
            self.as_setStateS({'bonusTypeSelected': updateVO['bonusTypeSelected'],
             ...
             'purchaseTypeSelectedIndex': updateVO['purchaseTypeSelectedIndex'],
             'enableGroupFilter': updateVO['enableGroupFilter']})
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\customization\filter_popover.pyc 
