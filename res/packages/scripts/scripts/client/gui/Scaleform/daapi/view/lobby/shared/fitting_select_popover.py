# 2017.08.29 21:47:37 Støední Evropa (letní èas)
...
def _extendByArtefactData(targetData, module, slotIndex):
    raise module.itemTypeID in GUI_ITEM_TYPE.ARTEFACTS or AssertionError
...
def _extendByOptionalDeviceData(targetData, module):
    raise module.itemTypeID == GUI_ITEM_TYPE.OPTIONALDEVICE or AssertionError
...
def _extendByBattleBoosterData(targetData, module, vehicle):
    if not module.itemTypeID == GUI_ITEM_TYPE.BATTLE_BOOSTER:
        raise AssertionError
        skillLearnt = module.isCrewBooster() and module.isAffectedSkillLearnt(vehicle)
...
def _getStatus(reason):
    if reason is not None and reason not in (GUI_ITEM_ECONOMY_CODE.ITEM_IS_HIDDEN, GUI_ITEM_ECONOMY_CODE.ITEM_IS_DUPLICATED):
...
class _PreviewLogicProvider(PopoverLogicProvider):
    ...
    def _buildModuleData(self, module, isInstalled, _):
        ...
        moduleData.update({'targetVisible': isInstalled,
         ...
         'removeButtonTooltip': MENU.MODULEFITS_REMOVETOOLTIP})
        return moduleData
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\shared\fitting_select_popover.pyc 
