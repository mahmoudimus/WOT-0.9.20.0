# 2017.08.29 21:47:41 Støední Evropa (letní èas)
...
class InventoryItemsTab(StoreItemsTab):
    ...
    def _getItemActionData(self, item):
        if item.sellPrices.itemPrice.isActionPrice() and not item.isRented:
...
class InventoryVehicleTab(InventoryItemsTab, StoreVehicleTab):
    ...
    def _getStatusImg(self, item):
        if item.isUnlocked:
            return getVehicleStateIcon(item.getState()[0])
        return ''
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\store\tabs\inventory.pyc 
