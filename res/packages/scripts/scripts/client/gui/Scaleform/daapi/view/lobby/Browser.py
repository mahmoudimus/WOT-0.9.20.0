# 2017.08.29 21:46:32 Støední Evropa (letní èas)
...
class Browser(BrowserMeta):
    ...
    def init(self, browserID, webHandlersMap = None, alias = ''):
        ...
        if not self.__browser is not None:
            ...
            self.__browser.hasBrowser or self.addListener(BrowserEvent.BROWSER_CREATED, self.__handleBrowserCreated)
        else:
    ...
    def __handleBrowserCreated(self, event):
        event.ctx['browserID'] == self.__browserID and self.removeListener(BrowserEvent.BROWSER_CREATED, self.__handleBrowserCreated)
        self.__browser = self.browserCtrl.getBrowser(self.__browserID)
        if not self.__browser is not None:
            raise AssertionError('Cannot find browser')
            self.__prepareBrowser()
    ...
    def __prepareBrowser(self):
        ...
        if self.__browser.isNavigationComplete:
            self.as_loadingStopS()
        return
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\Browser.pyc 
