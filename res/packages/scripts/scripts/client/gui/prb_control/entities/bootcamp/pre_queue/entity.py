...
class BootcampEntity(PreQueueEntity):
    ...
    def doSelectAction(self, action):
        if action.actionName == PREBATTLE_ACTION_NAME.BOOTCAMP:
            return SelectResult(True)
        return super(BootcampEntity, self).doSelectAction(action)
    ...
    def __onServerSettingChanged(self, diff):
        ...
        if not self.lobbyContext.getServerSettings().isBootcampEnabled():
            ...
            if self.isInQueue():
                ...
                __leave()
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\prb_control\entities\bootcamp\pre_queue\entity.pyc 
