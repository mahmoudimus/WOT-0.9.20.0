...
class Filter(object):
    ...
    def __isInDossier(self, item, installedInCurrentSlot):
        if not self.__showInDossier:
            return not item.isInDossier or installedInCurrentSlot
        return True
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\customization\filter.pyc 
