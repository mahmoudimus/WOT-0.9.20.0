    ...
    def installItem(self, itemCD, rootCD, state):
        ...
        if not vehicle.isInInventory:
            ...
            if item.isInInventory:
                ...
                processMsg(result)
                vehicle = result.success and item.itemTypeID in (GUI_ITEM_TYPE.TURRET, GUI_ITEM_TYPE.GUN) and self.itemsCache.items.getItemByCD(vehicle.intCD)
...
class BuyAndInstallItemVehicleLayout(SetVehicleLayoutAction):
    ...
    def doAction(self):
        ...
        return
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\shared\gui_items\items_actions\actions.pyc 
