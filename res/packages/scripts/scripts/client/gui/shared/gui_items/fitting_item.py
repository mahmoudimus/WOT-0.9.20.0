...
class FittingItem(GUIItem, HasIntCD):
    ...
    def mayObtainWithMoneyExchange(self, money, exchangeRate):
        ...
        if canRent:
            return True
        if self.isRestoreAvailable() and self.mayRestoreWithExchange(money, exchangeRate):
            return True
        return self.mayPurchaseWithExchange(money, exchangeRate)
    ...
    def __repr__(self):
        return '%s<intCD:%d, type:%s, nation:%d>' % (self.__class__.__name__,
         ...
         self.itemTypeName,
         self.nationID)
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\shared\gui_items\fitting_item.pyc 
