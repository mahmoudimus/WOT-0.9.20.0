...
class OptDeviceInstaller(ModuleInstallProcessor):
    ...
    def _successHandler(self, code, ctx = None):
        ...
        if not self.install and not self.item.isRemovable and self.useMoney:
            return makeI18nSuccess(self._formMessage('money_success'), type=smType, **self._getMsgCtx())
        else:
...
# 2017.08.29 21:49:53 Støední Evropa (letní èas)
