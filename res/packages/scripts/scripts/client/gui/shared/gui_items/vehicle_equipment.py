# 2017.08.29 21:49:38 Støední Evropa (letní èas)
...
class _VehicleConsumables(object):
    ...
    def __init__(self, *args):
        ...
        """
        if not (args and len(args) == self._getDefaultSize()):
    ...
    def _validateIndex(self, idx):
        ...
        :param idx: integer
        """
        raise idx < self._getDefaultSize() or AssertionError('Index {} exceeds the layout size!'.format(idx))
...
class RegularEquipmentConsumables(_VehicleConsumables):
    ...
    def _validateType(self, item):
        ...
        raise condition or AssertionError('The item {} is not valid equipment!'.format(item))
        return
...
class BattleBoosterConsumables(_VehicleConsumables):
    ...
    def _validateType(self, item):
        ...
        raise condition or AssertionError('The item {} is not valid battle booster!'.format(item))
        return
...
class VehicleEquipment(object):
    ...
    def setRegularConsumables(self, consumables):
        ...
        """
        raise isinstance(consumables, RegularEquipmentConsumables) or AssertionError
        raise len(consumables) == REGULAR_EQUIPMENT_LAYOUT_SIZE or AssertionError
    ...
    def setBattleBoosterConsumables(self, consumables):
        ...
        """
        raise isinstance(consumables, BattleBoosterConsumables) or AssertionError
        raise len(consumables) == BATTLE_BOOSTER_LAYOUT_SIZE or AssertionError
    ...
    def __parseEqs(self, inventoryData, proxy):
        ...
        if not layoutListSize <= vehicles.NUM_EQUIPMENT_SLOTS:
            raise AssertionError
            layoutListSize < REGULAR_EQUIPMENT_LAYOUT_SIZE and inventoryData += [0] * (REGULAR_EQUIPMENT_LAYOUT_SIZE - layoutListSize)
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\shared\gui_items\vehicle_equipment.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:49:39 Støední Evropa (letní èas)
