# 2017.08.29 21:49:23 Støední Evropa (letní èas)
...
def formatDiscount(discountVal, discountType):
    ...
    if discountType == DISCOUNT_TYPE.GOLD:
        return makeHtmlString('html_templates:lobby/quests/actions', Currency.GOLD, {'value': multiplier + BigWorld.wg_getGoldFormat(abs(long(discountVal)))})
...
def getMapName(arenaTypeID):
    if not (arenaTypeID not in ArenaType.g_cache and arenaTypeID in ArenaType.g_cache):
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\server_events\formatters.pyc 
