# 2017.08.29 21:49:24 Støední Evropa (letní èas)
...
class _BuyPriceMul(_PriceOpAbstract):
    ...
    def _getDiscountParams(self, item, value):
        ...
        return (_getPercentDiscountByMultiplier(value, item.buyPrices.itemPrice.defPrice.getSignValue(Currency.CREDITS)), _DT.PERCENT)
...
class EconomicsSet(ActionModifier):
    ...
    def _wrapParamName(self, name):
        if name == 'winXPFactorMode':
            return '{}/{}'.format(name, self.getParamValue())
        return name
    ...
    def handlerCreditsTankmanCost(self, value):
        ...
        if tankmanCost is not None:
            return self._calculateDiscount('creditsTankmanCost', value, tankmanCost[1][Currency.CREDITS], _DT.PERCENT)
        else:
            return float(value)
    ...
    def handlerGoldTankmanCost(self, value):
        tankmanCost = self.itemsCache.items.shop.defaults.tankmanCost
        if tankmanCost is not None:
            return self._calculateDiscount('goldTankmanCost', value, tankmanCost[2][Currency.GOLD], _DT.PERCENT)
        else:
            return float(value)
    ...
    def handlerCreditsDropSkillsCost(self, value):
        ...
        if dropSkillsCost is not None:
            return self._calculateDiscount('creditsDropSkillsCost', value, dropSkillsCost[1][Currency.CREDITS], _DT.PERCENT)
        else:
            return float(value)
    ...
    def handlerGoldDropSkillsCost(self, value):
        dropSkillsCost = self.itemsCache.items.shop.defaults.dropSkillsCost
        if dropSkillsCost is not None:
            return self._calculateDiscount('goldDropSkillsCost', value, dropSkillsCost[2][Currency.GOLD], _DT.PERCENT)
        else:
            return float(value)
...
def getModifierObj(name, params):
    if name in _MODIFIERS_DICT:
        ...
        return None
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\server_events\modifiers.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:49:26 Støední Evropa (letní èas)
