...
class ViewPointsController(IViewPointsController):
    ...
    def switch(self, isNext = True):
        ...
        if self.__doSwitch(switchToNext, items, currentItem):
            return True
        elif self.__doSelect(False, playerVehicleID):
            return True
        else:
    ...
    def __doSelect(self, isViewpoint, vehOrPointId):
        ...
        if isViewpoint:
            ...
            return True
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\battle_control\controllers\view_points_ctrl.pyc 
