# 2017.08.29 21:45:08 Støední Evropa (letní èas)
...
class _VehCompareData(object):
    ...
    def setCrewData(self, crewLvl, skills):
        raise crewLvl in CREW_TYPES.ALL or AssertionError('Unsupported crew level type: {}'.format(crewLvl))
...
class VehComparisonBasket(IVehicleComparisonBasket):
    ...
    def addVehicle(self, vehicleCompactDesr, initParameters = None):
        ...
        if not isinstance(vehicleCompactDesr, (int, float)):
            ...
            if self.__canBeAdded():
                ...
                vehCmpData and self.__vehicles.append(vehCmpData)
    ...
    def _createCache(self):
        ...
        if databaseID is not None:
            return _VehCmpCache(databaseID, self._applyVehiclesFromCache, self._getVehiclesIterator)
        else:
            return
    ...
    def __disposeCache(self):
        if self.__cache is not None:
            ...
            self.__cache = None
        return
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\game_control\veh_comparison_basket.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:45:09 Støední Evropa (letní èas)
