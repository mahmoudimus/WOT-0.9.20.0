# 2017.08.29 21:45:05 Støední Evropa (letní èas)
...
class RankedBattlesController(IRankedBattlesController, Notifiable):
    ...
    def isFrozen(self):
        ...
        if peripheryPrimeTime is not None and not peripheryPrimeTime.hasAnyPeriods():
            return True
        else:
            return False
    ...
    def getSeason(self, seasonID):
        if not isinstance(seasonID, int):
            raise AssertionError
            seasonData = self.__getSettings().seasons.get(seasonID, {})
            cycleInfo = seasonData and (None,
             None,
             seasonID,
             None)
    ...
    def getPrimeTimeStatus(self, peripheryID = None):
        ...
        if isNow:
            return (PRIME_TIME_STATUS.AVAILABLE, timeLeft)
        else:
            return (PRIME_TIME_STATUS.NOT_AVAILABLE, timeLeft)
    ...
    def getPrevRanks(self, accRank, vehRank, rankChange):
        ...
        return ranked_common.getPrevRanks(accRank, vehRank, rankChange, self.__getSettings().asDict(), {})
    ...
    def __getQuestForRank(self, rankId):
        ...
        if season is None:
            ...
            return first(self.eventsCache.getRankedQuests(lambda q: q.getRank() == rankId and q.isHidden() and q.isForRank() and q.getSeasonID() == season.getSeasonID() and q.getCycleID() == season.getCycleID()).values())
    ...
    def __getQuestForMaxRank(self, rankId):
        ...
        if season is None:
            ...
            return first(self.eventsCache.getRankedQuests(lambda q: q.getRank() == rankId and q.isHidden() and q.isProcessedAtCycleEnd() and q.getSeasonID() == season.getSeasonID() and q.getCycleID() == season.getCycleID()).values())
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\client\gui\game_control\ranked_battles_controller.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:45:06 Støední Evropa (letní èas)
