# 2017.08.29 21:57:06 Støední Evropa (letní èas)
...
class SearchDialogBase:
    ...
    def create_option_buttons(self):
        ...
        btn = Checkbutton(f, anchor='w', variable=self.engine.revar, text='Regular expression')
        btn.pack(side='left', fill='both')
        if self.engine.isre():
            btn.select()
        btn = Checkbutton(f, anchor='w', variable=self.engine.casevar, text='Match case')
        btn.pack(side='left', fill='both')
        if self.engine.iscase():
            btn.select()
        btn = Checkbutton(f, anchor='w', variable=self.engine.wordvar, text='Whole word')
        btn.pack(side='left', fill='both')
        if self.engine.isword():
            btn.select()
        if self.needwrapbutton:
            btn = Checkbutton(f, anchor='w', variable=self.engine.wrapvar, text='Wrap around')
            btn.pack(side='left', fill='both')
    ...
    def create_other_buttons(self):
        ...
        btn = Radiobutton(f, anchor='w', variable=self.engine.backvar, value=1, text='Up')
        btn.pack(side='left', fill='both')
        if self.engine.isback():
            btn.select()
        btn = Radiobutton(f, anchor='w', variable=self.engine.backvar, value=0, text='Down')
        btn.pack(side='left', fill='both')
    ...
    def create_command_buttons(self):
        ...
        b.lower()
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\idlelib\SearchDialogBase.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:57:07 Støední Evropa (letní èas)
