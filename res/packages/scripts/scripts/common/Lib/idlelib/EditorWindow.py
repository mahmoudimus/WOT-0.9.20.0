...
class EditorWindow(object):
    ...
    def __init__(self, flist = None, filename = None, key = None, root = None):
        ...
        if idleConf.GetOption('main', 'EditorWindow', 'font-bold', type='bool'):
            fontWeight = 'bold'
        text.config(font=(idleConf.GetOption('main', 'EditorWindow', 'font'), idleConf.GetOption('main', 'EditorWindow', 'font-size', type='int'), fontWeight))
    ...
    rmenu_specs = [('Close', '<<close-window>>', None)]
    ...
    def _asktabwidth(self):
        return self.askinteger('Tab width', 'Columns per tab? (2-16)', parent=self.text, initialvalue=self.indentwidth, minvalue=2, maxvalue=16)
...
def get_accelerator(keydefs, eventname):
    ...
    if not keylist or macosxSupport.isCocoaTk() and eventname in {'<<open-module>>', '<<goto-line>>', '<<change-indentwidth>>'}:
        return ''
    s = keylist[0]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) <= 1:
        run(_Help_dialog)
    run(_Editor_window)
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\idlelib\EditorWindow.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:56:57 Støední Evropa (letní èas)
