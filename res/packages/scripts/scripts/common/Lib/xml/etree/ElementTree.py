# 2017.08.29 21:59:20 Støední Evropa (letní èas)
    ...
    def write(self, file_or_filename, encoding = None, xml_declaration = None, default_namespace = None, method = None):
        ...
        if not encoding:
            if method == 'c14n':
                ...
                encoding = 'us-ascii'
        elif xml_declaration or xml_declaration is None and encoding not in ('utf-8', 'us-ascii'):
            if method == 'xml':
            ...
            if items or namespaces:
                if namespaces:
                    for v, k in sorted(namespaces.items(), key=lambda x: x[1]):
                        if k:
                            k = ':' + k
                        write(' xmlns%s="%s"' % (k.encode(encoding), _escape_attrib(v, encoding)))
    ...
    def _default(self, text):
        ...
        if prefix == '&':
            try:
                ...
                from xml.parsers import expat
                err = expat.error('undefined entity %s: line %d, column %d' % (text, self._parser.ErrorLineNumber, self._parser.ErrorColumnNumber))
                err.code = 11
...
try:
    ...
    pass
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\xml\etree\ElementTree.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:59:21 Støední Evropa (letní èas)
