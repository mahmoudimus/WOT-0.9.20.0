# 2017.08.29 21:58:31 Støední Evropa (letní èas)
...
class Application():
    ...
    def do_about(self, id, item, window, event):
        EasyDialogs.Message('Hello, world!' + '\r(%s)' % self.__class__.__name__)
    ...
    def do_unknownpartcode(self, partcode, window, event):
        ...
        if DEBUG:
            print '\tEvent:', self.printevent(event)
        if hasattr(MacOS, 'HandleEvent'):
            MacOS.HandleEvent(event)
...
class MenuBar():
    ...
    def dispatch(self, id, item, window, event):
        if id in self.menus:
            ...
            print 'MenuBar.dispatch(%d, %d, %s, %s)' % (id,
             item,
             window,
             event)
...
class Window():
    ...
    def open(self, bounds = (40, 40, 400, 400), resid = None):
        if resid != None:
            ...
            self.wid = NewWindow(bounds, self.__class__.__name__, 1, 8, -1, 1, 0)
        self.do_postopen()
        return
    ...
    def do_inZoom(self, partcode, window, event):
        ...
        if window.TrackBox(where, partcode):
            ...
            rect = window.GetWindowUserState()
            self.do_postresize(rect[2] - rect[0], rect[3] - rect[1], window)
    ...
    def do_inGrow(self, partcode, window, event):
        ...
        if result:
            height = result >> 16 & 65535
            width = result & 65535
            self.do_resize(width, height, window)

    growlimit = (50,
     50,
     screenbounds[2] - screenbounds[0],
     screenbounds[3] - screenbounds[1])

    def do_resize(self, width, height, window):
        l, t, r, b = self.wid.GetWindowPort().GetPortBounds()
        self.SetPort()
        self.wid.InvalWindowRect((r - SCROLLBARWIDTH + 1,
         b - SCROLLBARWIDTH + 1,
         r,
         b))
        window.SizeWindow(width, height, 1)
    ...
    def do_activate(self, activate, event):
        if DEBUG:
            print 'Activate %d for %s' % (activate, self.wid)
...
class ControlsWindow(Window):
    ...
    def do_rawupdate(self, window, event):
        ...
        window.BeginUpdate()
        self.do_update(window, event)
        UpdateControls(window, window.GetWindowPort().visRgn)
        window.DrawGrowIcon()
        window.EndUpdate()
    ...
    def do_inContent(self, partcode, window, event):
        ...
        if pcode and control:
            ...
            if DEBUG:
                print 'FindControl(%s, %s) -> (%s, %s)' % (local,
                 window,
                 pcode,
                 control)
            self.do_contentclick(local, modifiers, event)
...
class ScrolledWindow(ControlsWindow):
    ...
    def scrollbars(self, wantx = 1, wanty = 1):
        ...
        if vx is None:
            self.barx_enabled, vx = (0, 0)
        if vy is None:
            self.bary_enabled, vy = (0, 0)
        if wantx:
            rect = (x0 - 1,
             y1 - (SCROLLBARWIDTH - 1),
             x1 - (SCROLLBARWIDTH - 2),
    ...
    def do_activate(self, onoff, event):
        ...
        if onoff:
            if self.barx and self.barx_enabled:
                self.barx.ShowControl()
            if self.bary and self.bary_enabled:
                self.bary.ShowControl()
        else:
            if self.barx:
                self.barx.HideControl()
            if self.bary:
                self.bary.HideControl()
    ...
    def do_postresize(self, width, height, window):
        ...
        if self.barx:
            ...
            self.barx.SizeControl(r - l - (SCROLLBARWIDTH - 3), SCROLLBARWIDTH)
        if self.bary:
            self.bary.HideControl()
            self.bary.MoveControl(r - (SCROLLBARWIDTH - 1), t - 1)
            self.bary.SizeControl(SCROLLBARWIDTH, b - t - (SCROLLBARWIDTH - 3))
        if self.barx:
            self.barx.ShowControl()
            self.wid.ValidWindowRect((l,
             b - SCROLLBARWIDTH + 1,
             r - SCROLLBARWIDTH + 2,
             b))
        if self.bary:
            self.bary.ShowControl()
            self.wid.ValidWindowRect((r - SCROLLBARWIDTH + 1,
             t,
             r,
             b - SCROLLBARWIDTH + 2))
        self.wid.InvalWindowRect((r - SCROLLBARWIDTH + 1,
...
if __name__ == '__main__':
    test()
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\plat-mac\FrameWork.pyc 
