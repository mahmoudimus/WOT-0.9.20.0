# 2017.08.29 21:58:33 Støední Evropa (letní èas)
...
def main():
    if len(sys.argv) > 1:
        ...
        for filename in args:
            process_func(filename, output=output, basepkgname=basepkgname, edit_modnames=edit_modnames, creatorsignature=creatorsignature, dump=dump, verbose=verbose)
...
def main_interactive(interact = 0, basepkgname = 'StdSuites'):
    ...
    filename = EasyDialogs.AskFileForOpen(message='Select scriptable application', dialogOptionFlags=4182, defaultLocation=appsfolder)
    if not filename:
        return
    else:
        if not is_scriptable(filename):
            if EasyDialogs.AskYesNoCancel('Warning: application does not seem scriptable', yes='Continue', default=2, no='') <= 0:
                return
        try:
            processfile(filename, edit_modnames=edit_modnames, basepkgname=basepkgname, verbose=sys.stderr)
        except MacOS.Error as arg:
            print 'Error getting terminology:', arg
            print 'Retry, manually parsing resources'
            processfile_fromresource(filename, edit_modnames=edit_modnames, basepkgname=basepkgname, verbose=sys.stderr)
...
getargument = [(getpstr, 'name'), (getostype, 'keyword'), (getdata, 'what')]
getevent = [(getpstr, 'name'),
 (getpstr, 'description'),
...
getproperty = [(getpstr, 'name'), (getostype, 'code'), (getdata, 'what')]
getelement = [(getostype, 'type'), (getlist, 'keyform', getostype)]
getclass = [(getpstr, 'name'),
 (getostype, 'class code'),
 (getpstr, 'description'),
 (getlist, 'properties', getproperty),
 (getlist, 'elements', getelement)]
getcomparison = [(getpstr, 'operator name'), (getostype, 'operator ID'), (getpstr, 'operator comment')]
getenumerator = [(getpstr, 'enumerator name'), (getostype, 'enumerator ID'), (getpstr, 'enumerator comment')]
getenumeration = [(getostype, 'enumeration ID'), (getlist, 'enumerator', getenumerator)]
getsuite = [(getpstr, 'suite name'),
...
getaete = [(getword, 'major/minor version in BCD'),
...
class SuiteCompiler():
    ...
    def precompilesuite(self):
        ...
        if self.edit_modnames is None:
            self.pathname = EasyDialogs.AskFileForSave(message='Python output file', savedFileName=modname + '.py')
        else:
        ...
        if not self.pathname:
            ...
            objc = ObjectCompiler(None, self.modname, basemodule, interact=self.edit_modnames is None, verbose=self.verbose)
            for cls in classes:
    ...
    def compileevent(self, event):
        ...
        if has_arg:
            if not opt_arg:
                ...
                fp.write('_object=None, ')
        else:
            fp.write('_no_object=None, ')
...
class ObjectCompiler():
    ...
    def fillclasspropsandelems(self, cls):
        ...
        if self.namemappers[0].hascode('class', code) and self.namemappers[0].findcodename('class', code)[0] != cname:
            if self.fp and (elements or (len(properties) > 1 or len(properties) == 1) and properties[0][1] != 'c@#!'):
                if self.verbose:
...
if __name__ == '__main__':
    main()
    sys.exit(1)
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\plat-mac\gensuitemodule.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:58:34 Støední Evropa (letní èas)
