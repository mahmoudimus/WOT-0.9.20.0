# 2017.08.29 21:57:29 Støední Evropa (letní èas)
...
class AbstractWidgetTest(object):
    ...
    def checkEnumParam(self, widget, name, *values, **kwargs):
        ...
        self.checkParams(widget, name, *values, **kwargs)
        if errmsg is None:
            errmsg2 = ' %s "{}": must be %s%s or %s' % (name,
             ', '.join(values[:-1]),
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\lib-tk\test\widget_tests.pyc 
