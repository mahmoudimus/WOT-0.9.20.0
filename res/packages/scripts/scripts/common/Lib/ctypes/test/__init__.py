# 2017.08.29 21:55:52 Støední Evropa (letní èas)
...
class TestRunner(unittest.TextTestRunner):
    ...
    def run(self, test, skipped):
        ...
        if _unavail:
            ...
            requested.sort()
            self.stream.writeln('Ran %d test%s in %.3fs (%s module%s skipped)' % (run,
             run != 1 and 's' or '',
             timeTaken,
             len(skipped),
...
class BasicTestRunner:
    ...
    def run(self, test):
        ...
        test(result)
        return result
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\ctypes\test\__init__.pyc 
