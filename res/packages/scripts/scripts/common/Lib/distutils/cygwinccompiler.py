# 2017.08.29 21:55:56 Støední Evropa (letní èas)
...
class Mingw32CCompiler(CygwinCCompiler):
    ...
    def __init__(self, verbose = 0, dry_run = 0, force = 0):
        ...
        if self.gcc_version < '4' or is_cygwingcc():
            ...
            no_cygwin = ''
        self.set_executables(compiler='gcc%s -O -Wall' % no_cygwin, compiler_so='gcc%s -mdll -O -Wall' % no_cygwin, compiler_cxx='g++%s -O -Wall' % no_cygwin, linker_exe='gcc%s' % no_cygwin, linker_so='%s%s %s %s' % (self.linker_dll,
         no_cygwin,
         shared_option,
         entry_point))
        self.dll_libraries = []
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\distutils\cygwinccompiler.pyc 
