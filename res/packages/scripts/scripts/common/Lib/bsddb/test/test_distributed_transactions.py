...
class DBTxn_distributed(unittest.TestCase):
    ...
    def _create_env(self, must_open_db):
        ...
        self.dbenv.open(self.homeDir, db.DB_CREATE | db.DB_THREAD | db.DB_RECOVER | db.DB_INIT_TXN | db.DB_INIT_LOG | db.DB_INIT_MPOOL | db.DB_INIT_LOCK, 438)
        self.db = db.DB(self.dbenv)
        self.db.set_re_len(db.DB_GID_SIZE)
        if must_open_db:
            txn = self.dbenv.txn_begin()
            self.db.open(self.filename, db.DB_QUEUE, db.DB_CREATE | db.DB_THREAD, 438, txn=txn)
            txn.commit()
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\bsddb\test\test_distributed_transactions.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:55:26 Støední Evropa (letní èas)
