# 2017.08.29 21:55:25 Støední Evropa (letní èas)
...
class TableDBTestCase(unittest.TestCase):
    ...
    def test01(self):
        ...
        if sys.version_info[0] < 3:
            ...
            self.tdb.Insert(tabname, {colname: pickle.dumps(3.14159, 1).decode('iso8859-1')})
        if verbose:
            self.tdb._db_print()
    ...
    def test02(self):
        ...
        import sys
        if sys.version_info[0] < 3:
            testinfo = [{col0: pickle.dumps(8, 1),
              col1: 'no',
              col2: 'Penguin'}, {col0: pickle.dumps(-1, 1),
              col1: 'no',
              col2: 'Turkey'}, {col0: pickle.dumps(9, 1),
              col1: 'yes',
              col2: 'SR-71A Blackbird'}]
        else:
            testinfo = [{col0: pickle.dumps(8, 1).decode('iso8859-1'),
              col1: 'no',
              col2: 'Penguin'}, {col0: pickle.dumps(-1, 1).decode('iso8859-1'),
              col1: 'no',
              col2: 'Turkey'}, {col0: pickle.dumps(9, 1).decode('iso8859-1'),
              col1: 'yes',
              col2: 'SR-71A Blackbird'}]
        try:
            self.tdb.Drop(tabname)
        ...
        if sys.version_info[0] < 3:
            ...
            values = self.tdb.Select(tabname, [col2], conditions={col0: lambda x: pickle.loads(bytes(x, 'iso8859-1')) >= 8})
    ...
    def test03(self):
        ...
        if verbose:
            ...
            self.tdb._db_print()
        self.tdb.CreateTable(tabname, ['a',
         'b',
         'c',
         'd',
         'e'])
        ...
        self.tdb.Insert(tabname, {'a': '42',
         'b': 'bad',
         'c': 'meep',
         'e': 'Fuzzy wuzzy was a bear'})
        self.tdb.Insert(tabname, {'a': '581750',
         'b': 'good',
         'd': 'bla',
         'c': 'black',
         'e': 'fuzzy was here'})
        self.tdb.Insert(tabname, {'a': '800000',
         'b': 'good',
         'd': 'bla',
         'c': 'black',
        ...
        if verbose:
            self.tdb._db_print()
        values = self.tdb.Select(tabname, ['b', 'a', 'd'], conditions={'e': re.compile('wuzzy').search,
    ...
    def test04_MultiCondSelect(self):
        ...
        if verbose:
            self.tdb._db_print()
        values = self.tdb.Select(tabname, ['b', 'a', 'd'], conditions={'e': dbtables.ExactCond('E'),
    ...
    def test_CreateOrExtend(self):
        ...
        self.tdb.CreateOrExtendTable(tabname, ['name',
         'taste',
         'filling',
         'alcohol content',
         'price'])
        try:
            self.tdb.Insert(tabname, {'taste': 'crap',
             'filling': 'no',
             'is it Guinness?': 'no'})
        ...
        self.tdb.Insert(tabname, {'taste': 'crap',
         'filling': 'no',
         'is it Guinness?': 'no'})
        self.tdb.Insert(tabname, {'taste': 'great',
         'filling': 'yes',
         'is it Guinness?': 'yes',
         'name': 'Guinness'})
    ...
    def test_CondObjs(self):
        ...
        values = self.tdb.Select(tabname, None, {'b': dbtables.Cond(),
    ...
    def test_Delete(self):
        ...
        self.tdb.Insert(tabname, {'x': 'X1',
         'y': 'Y1'})
        self.tdb.Insert(tabname, {'x': 'X2',
         'y': 'Y2',
         'z': 'Z2'})
    ...
    def test_Modify(self):
        ...
        self.tdb.Insert(tabname, {'Name': 'Index to MP3 files.doc',
         'Type': 'Word',
         'Access': '8'})
        self.tdb.Insert(tabname, {'Name': 'Nifty.MP3',
         'Access': '1'})
        self.tdb.Insert(tabname, {'Type': 'Unknown',
         'Access': '0'})
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\bsddb\test\test_dbtables.pyc 
