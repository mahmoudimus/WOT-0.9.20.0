# 2017.08.29 21:55:21 Støední Evropa (letní èas)
...
class BasicTestCase(unittest.TestCase):
    ...
    def test03_SimpleCursorStuff(self, get_raises_error = 0, set_raises_error = 0):
        ...
        while rec is not None:
            ...
            self.assertEqual(c.get_current_size(), len(c.current()[1]), '%s != len(%r)' % (c.get_current_size(), c.current()[1]))
    ...
    def test04_PartialGetAndPut(self):
        ...
        self.assertEqual(d.get(key, dlen=20, doff=990), '1111111111' + '2222222222')
    ...
    if db.version() >= (4, 7):
        ...
        def test_compact(self):
            ...
            d.delete('abc')
            d.delete('bcde')
            d.compact(start='abcde', stop='monty', txn=None, compact_fillpercent=42, compact_pages=1, compact_timeout=50000000, flags=db.DB_FREELIST_ONLY | db.DB_FREE_SPACE)
            return
...
class BasicTransactionTestCase(BasicTestCase):
    ...
    def test06_Transactions(self):
        ...
        self.assertEqual(d.get('new rec', txn=self.txn), 'this is a new record')
        self.txn.abort()
...
class BasicDUPTestCase(BasicTestCase):
    ...
    def test10_DuplicateKeys(self):
        ...
        if verbose:
            print '\n', '-=' * 30
            print 'Running %s.test10_DuplicateKeys...' % self.__class__.__name__
        d.put('dup0', 'before')
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\bsddb\test\test_basics.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:55:22 Støední Evropa (letní èas)
