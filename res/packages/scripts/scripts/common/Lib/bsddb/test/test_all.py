# 2017.08.29 21:55:20 Støední Evropa (letní èas)
...
if sys.version_info[0] >= 3:
    ...
    class DB_py3k(object):
        ...
        def get(self, key, default = 'MagicCookie', txn = None, flags = 0, dlen = -1, doff = -1):
            ...
            if default != 'MagicCookie':
                v = self._db.get(key, default=default, txn=txn, flags=flags, dlen=dlen, doff=doff)
            else:
    ...
    class DBEnv_py3k(object):
        ...
        def get_data_dirs(self):
            return tuple((i.decode(charset) for i in self._dbenv.get_data_dirs()))
...
try:
    ...
    from bsddb import db, dbtables, dbutils, dbshelve, hashopen, btopen, rnopen, dbobj
...
def get_new_path(name):
    ...
    try:
        import os
        path = os.path.join(get_new_path.prefix, name + '_' + str(os.getpid()) + '_' + str(get_new_path.num))
        get_new_path.num += 1
    finally:
...
import os, os.path
get_new_path.prefix = os.path.join(os.environ.get('TMPDIR', os.path.join(os.sep, 'tmp')), 'z-Berkeley_DB')
get_new_path.num = 0
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\bsddb\test\test_all.pyc 
