...
class ComparatorTests(unittest.TestCase):
    ...
    def comparator_test_helper(self, comparator, expected_data):
        ...
        if sys.version_info < (2, 6):
            ...
            data = data2
        self.assertEqual(data, expected_data, "comparator `%s' is not right: %s vs. %s" % (comparator, expected_data, data))
    ...
    def check_results(self, expected):
        ...
        try:
            ...
            while rec:
                ...
                self.assertEqual(expected[index], key, "expected value `%s' at %d but got `%s'" % (expected[index], index, key))
                index = index + 1
                rec = curs.next()
    ...
    def check_results(self, expected):
        ...
        try:
            ...
            while rec:
                ignore, data = rec
                self.assertLess(index, len(expected), 'to many values returned from cursor')
                self.assertEqual(expected[index], data, "expected value `%s' at %d but got `%s'" % (expected[index], index, data))
                index = index + 1
                rec = curs.next()
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\bsddb\test\test_compare.pyc 
