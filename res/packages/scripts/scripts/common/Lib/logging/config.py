            ...
            try:
                ...
                if len(chunk) == 4:
                    ...
                    try:
                        import json
                        d = json.loads(chunk)
                        raise isinstance(d, dict) or AssertionError
                        dictConfig(d)
                    except:
...
def stopListening():
    ...
    return
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\Lib\logging\config.pyc 
