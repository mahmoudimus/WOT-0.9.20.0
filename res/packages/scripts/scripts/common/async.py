# 2017.08.29 21:52:23 Støední Evropa (letní èas)
...
class _Future(object):
    ...
    def set_result(self, result):
        ...
        if self.__expired:
            ...
            self.__cancel_timeout()
            if not not self.__result_set:
                raise AssertionError
                self.__result_set = True
                callback = self.__callback is not None and self.__callback
                self.__callback = None
                callback(result)
            else:
    ...
    def then(self, callback):
        ...
        """
        if not not self.__callback_set:
            raise AssertionError
            self.__callback_set = True
            result = self.__result_set and self.__result
            self.__result = None
            callback(result)
        else:
    ...
    def set_timeout(self, timeout):
        if not self.__timerID is None:
            raise AssertionError
            raise timeout >= 0 or AssertionError
            self.__timerID = self.__result_set or BigWorld.addTimer(self.__expire, timeout)
        return
...
class _Promise(object):
    ...
    def set_value(self, value):
        ...
        """
        if not not self.__value_set:
            raise AssertionError
            self.__value_set = True
            self.__cancel = None
            future = self.__future
            self.__future = future is not None and None
    ...
    def get_future(self):
        ...
        """
        if not not self.__future_set:
            raise AssertionError
            self.__future_set = True
            future = _Future(self)
            self.__value_set and future.set_result(_FulfilledPromiseResult(self.__value, self.__exc_info))
...
class AsyncQueue(AsyncObject):
    ...
    def destroy(self):
        del self.__promises
        del self.__values
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\async.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:52:24 Støední Evropa (letní èas)
