...
def generateCompactDescr(passport, vehicleTypeID, role, roleLevel, skills = (), lastSkillLevel = MAX_SKILL_LEVEL, dossierCompactDescr = '', freeSkills = ()):
    pack = struct.pack
    if not MIN_ROLE_LEVEL <= roleLevel <= MAX_SKILL_LEVEL:
        ...
        levelsToNextRank = rank == maxRankIdx and 0
...
class TankmanDescr(object):
    ...
    def efficiencyOnVehicle(self, vehicleDescr):
        ...
        raise nationID == self.nationID or AssertionError
    ...
    def __initFromCompactDescr(self, compactDescr, battleOnly):
        ...
        try:
            ...
            if not header & 15 == ITEM_TYPES.tankman:
                ...
                raise nationConfig.hasIcon(self.iconID) or KeyError(self.iconID)
        except Exception:
...
# okay decompyling c:\Users\PC\wotmods\files\originals\res\packages\scripts\scripts\common\items\tankmen.pyc 
# decompiled 1 files: 1 okay, 0 failed, 0 verify failed
# 2017.08.29 21:53:18 Støední Evropa (letní èas)
